...
procedure RotateAndTranslate(var rx, ry: double; const px, py, tx, ty, st, ct: double); inline;
...
function avgAngle(a1, a2, alpha: double): double;
...
var
    a1, a2, b1, b2, c1, c2 : double; // line equantions coefficients
    denom : double;
...
// Rotacao do vector (px,py) do angulo teta (dado por sin, cos)seguida de
// Translacao segundo o vector (tx,ty)
procedure RotateAndTranslate(var rx, ry: double; const px, py, tx, ty, st, ct: double);
begin
  rx := px * ct - py * st + tx;
  ry := px * st + py * ct + ty;
end;
...
function avgAngle(a1, a2, alpha: double): double;
var vx1, vy1, vx2, vy2, rx, ry: double;
    teta, st, sat, s1at: double;
begin
  teta := DiffAngle(a1, a2);
  if abs(teta) < 1e-6 then begin
    result := a1;
    exit;
  end;
  ...
  st := sin(teta);
  s1at := sin((1-alpha)*teta);
  sat := sin(alpha*teta);
  rx := s1at * vx1 / st + sat * vx2 / st;
  ry := s1at * vy1 / st + sat * vy2 / st;
