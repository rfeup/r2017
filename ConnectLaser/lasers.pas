...
uses
  ...
  TASeries, SynEdit, lNetComponents, math, utils, LCLIntf, Rlan;
...
type
 ...
 TNeatoRays = array[0..MaxNeatoScans-1] of TNeatoRay;
  ...
  TFLasers = class(TForm)
    ...
    procedure EditComPortNeatoChange(Sender: TObject);
...
procedure TFLasers.Edit1Change(Sender: TObject);
begin

end;

procedure TFLasers.EditComPortNeatoChange(Sender: TObject);
begin
          ...
          for i := 0 to LaserScanNeato.count - 1 do begin
            if not NeatoRays[i].invalid_data then begin
              ...
              if LaserScanNeato.Rays[i].d > 0 then begin
                  ...
                  a1:= 1.03429716705016;//1.037039514388;
              ...
              //LaserScanNeato.Rays[i].d := -0.0262756318238165*(LaserScanNeato.Rays[i].d*LaserScanNeato.Rays[i].d)+1.03649062495971*(LaserScanNeato.Rays[i].d)-0.0412124119851335;
            end else begin
            ...
            if CBDebug.Checked then begin
              measure := measure + floattostr(LaserScanNeato.Rays[i].d) + ',';
            end;
          end;

          if CBDebug.Checked then begin
            if FLasers.Memo.Lines.Count < 259 then begin
              FLasers.Memo.Lines.addText(measure);
              EditDebug.text:= inttostr(FLasers.Memo.Lines.Count);
            end;
          end;
...
procedure TFLasers.Button1Click(Sender: TObject);
begin
  FLasers.Memo.Lines.SaveToFile('distance'+Edit1.text+'.csv');
end;
